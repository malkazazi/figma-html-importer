...
ZIP="$ROOT/figma-html-importer-${VERSION}.zip"
mkdir -p "$STAGE/plugin"

# Only what a user needs to INSTALL — built output, manifests, docs.
...
cp README.md INSTALL.txt "$STAGE/"
...
DL_URL="$(gh release view "$VERSION" --json assets --jq '.assets[0].url')"
echo ""
echo "✓ Released ${VERSION}"
echo "  Share this link with designers: ${DL_URL}"
